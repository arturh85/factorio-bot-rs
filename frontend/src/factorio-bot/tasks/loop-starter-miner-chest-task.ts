...
const TASK_TYPE = 'loop-starter-miner-chest'
...
async function executeThisTask(store: Store<State>, bots: FactorioBot[], task: Task): Promise<void> {
    ...
    const bot = bots[0]
    let remaining = data.oreCount
    while (remaining > 0) {
        store.commit('updateTask', updateTaskStatus(task, TaskStatus.STARTED));
        const inventoryContents = await FactorioApi.inventoryContentsAt((store.state.world.starterMinerChests || [])
            .filter(minerChest => minerChest.oreName === data.oreName)
            .flatMap(minerChest => {
        ...
        for (const entity of inventoryContents) {
            if (entity.outputInventory && entity.outputInventory[data.oreName]) {
                const take = Math.min(remaining, entity.outputInventory[data.oreName])
                await bot.removeFromInventory(entity.name, entity.position, InventoryType.chest_or_fuel, data.oreName, take)
                remaining -= take
                if (remaining === 0) {
                    break
                }
        ...
        const lowFuelEntities = inventoryContents.filter(entity => entity.name === Entities.burnerMiningDrill &&
            (entity.fuelInventory && (!entity.fuelInventory[data.fuelName] || entity.fuelInventory[data.fuelName] < targetFuel)))

        const lowFuelEntitiesFuel = lowFuelEntities.reduce((coal, entity) => {
            return coal + ((entity.fuelInventory || {})[data.fuelName] || 0)
        }, 0)
        const fuelNeeded = lowFuelEntities.length * targetFuel - lowFuelEntitiesFuel - bot.mainInventory(data.fuelName)
        ...
        // then insert fuel
        for (const entity of lowFuelEntities) {
...
export async function createLoopStarterMinerChestTask(store: Store<State>, fuelName: string, oreName: string, oreCount: number): Promise<Task> {
    const data: TaskData = {
        fuelName,
        oreName: oreName,
        oreCount: oreCount,
    }
    return createTask(TASK_TYPE, `Loop Starter Miner/Chest for ${oreName} x ${oreCount}`, data)
}